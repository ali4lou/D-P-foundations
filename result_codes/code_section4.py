...
# embryo_times_ks_test=np.loadtxt(path_save_data+'embryo_times_ks_test.txt', dtype=float)
# embryo_times_p_value=np.loadtxt(path_save_data+'embryo_times_p_value.txt', dtype=float)
...
#     df.to_csv(path_save_data+'important_phen_clust_time_%d.csv' %i, sep='\t')
...
#8.1.a) We perform a second clustering
...
for i in range(len(inner_clust_cell_type)):
    ...
    plt.savefig(path_save_data+'big_cell_types_cluster_%s.png' %label_clust[i], dpi=600, bbox_inches='tight')
    plt.show()
...
plt.yticks(fontsize=16)
plt.savefig(path_save_data+'frac_precursor_cells.png', dpi=600, bbox_inches='tight')
plt.show()
...
for i in range(len(inner_clust)):
    ...
    df.to_csv(path_save_data+'important_phen_clust_cell_type_%d.csv' %i, sep='\t')
...
p50=np.median(ks_signif_cell_types_per_phen)
plt.figure(figsize=(4, 3), dpi=600)
plt.hist(ks_signif_cell_types_per_phen, bins=50, color='mediumseagreen')
plt.xlabel('# significant KS \n (Cell types)', fontsize=20, fontweight='bold')
plt.ylabel('# NMF phen', fontsize=20, fontweight='bold')
plt.axvline(x=p50, color='darkgreen', linestyle='--')
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.savefig(path_save_data+'ks_signif_cell_type_per_phen.png',dpi=600, bbox_inches='tight')



#ks signif per cell type
ks_signif_phen_per_cell_type=np.zeros(len(new_ks_signif[0, :]))
for i in range(len(new_ks_signif[0, :])):
    ind_not_null=np.where(new_ks_signif[:, i]>0)[0]
    ks_signif_phen_per_cell_type[i]=len(ind_not_null)
    
# pearsonr(nmf_comp_pleio, ks_signif_cell_types_per_phen)
pearsonr(cell_type_pleio, ks_signif_phen_per_cell_type)



np.median(ks_signif_phen_per_cell_type)
# np.mean(ks_signif_cell_types_per_phen)
# pearsonr(average_ks, ks_signif_cell_types_per_phen)

p50=np.median(ks_signif_phen_per_cell_type)
plt.figure(figsize=(4, 3), dpi=600)
plt.hist(ks_signif_phen_per_cell_type, bins=50, color='royalblue')
plt.xlabel('# significant KS \n (NMF phen)', fontsize=20, fontweight='bold')
plt.ylabel('# cell types', fontsize=20, fontweight='bold')
plt.axvline(x=p50, color='indigo', linestyle='--')
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.savefig(path_save_data+'ks_signif_phen_per_cell_type.png',dpi=600, bbox_inches='tight')
...
most_imp_phen_final=[]
#busco phen concretos y sus ols phen
comp_analyze_NMF=df_nmf_pleio_high_ks['NMF phen']
for i in range(len(comp_analyze_NMF)):
    ind=int(comp_analyze_NMF[i])
    ind_h=np.argsort(H[ind, :])[::-1]
    most_imp_phen_sorted=phen[ind_h]
    most_imp_phen_final.append(most_imp_phen_sorted[:5])
    # print('\n')
    
import csv
with open(path_save_data+"most_imp_phen_final.csv", mode="w", newline="") as file:
    writer = csv.writer(file)
    writer.writerows(most_imp_phen_final)



#la componente 49 está en phen_sorted[1]
comp_49_sorted=np.argsort(new_ks_signif[1, :])[::-1]
important_cells_comp49=new_cell_type[comp_49_sorted]
ks_associated=new_ks_signif[1, comp_49_sorted]
