...
#3.5.3.A.) We search for the outstanding coordinates for highly and lowly pleio genes

#less pleio > high pleio & less pleio>40%

# chosen_index_genes_high_pleio = np.where((frac_genes_high_pleio_signif_per_coord < 0.2) & (frac_genes_high_pleio_signif_per_coord > 0))[0]
# chosen_index_genes_less_pleio = np.where(frac_genes_less_pleio_signif_per_coord>0.05)[0]

# common_index=np.intersect1d(chosen_index_genes_high_pleio, chosen_index_genes_less_pleio)
# good_index_less_pleio=[]
# time_less_pleio_sig_zscore=[]
# cell_less_pleio_sig_zscore=[]
# for i in range(len(common_index)):
#     if frac_genes_high_pleio_signif_per_coord[int(common_index[i])]<frac_genes_less_pleio_signif_per_coord[int(common_index[i])]:
#         time_less_pleio_sig_zscore.append(time[int(np.trunc(common_index[i]/len(cell_types)))])
#         cell_less_pleio_sig_zscore.append(cell_types[int(common_index[i]%len(cell_types))])
#         good_index_less_pleio.append(common_index[i])
        
# cell_less_pleio_sig_zscore=np.array(cell_less_pleio_sig_zscore)

# df_less_pleio=pd.DataFrame()
# df_less_pleio['time']=time_less_pleio_sig_zscore
# df_less_pleio['cell_type']=cell_less_pleio_sig_zscore
# df_less_pleio.to_excel(path_save_data+'df_less_pleio_coord_outstanding.xlsx', index=False)

# #high pleio > less pleio & high pleio>40%
# chosen_index_genes_high_pleio = np.where(frac_genes_high_pleio_signif_per_coord > 0.4)[0]
# chosen_index_genes_less_pleio = np.where(frac_genes_less_pleio_signif_per_coord>0)[0]

# common_index=np.intersect1d(chosen_index_genes_high_pleio, chosen_index_genes_less_pleio)
# good_index_high_pleio=[]
# time_high_pleio_sig_zscore=[]
# cell_high_pleio_sig_zscore=[]
# for i in range(len(common_index)):
#     if frac_genes_high_pleio_signif_per_coord[int(common_index[i])]>frac_genes_less_pleio_signif_per_coord[int(common_index[i])]:
#         time_high_pleio_sig_zscore.append(time[int(np.trunc(common_index[i]/len(cell_types)))])
#         cell_high_pleio_sig_zscore.append(cell_types[int(common_index[i]%len(cell_types))])
#         good_index_high_pleio.append(common_index[i])
        
# cell_high_pleio_sig_zscore=np.array(cell_high_pleio_sig_zscore)


# df_high_pleio=pd.DataFrame()
# df_high_pleio['time']=time_high_pleio_sig_zscore
# df_high_pleio['cell_type']=cell_high_pleio_sig_zscore
# df_high_pleio.to_excel(path_save_data+'df_high_pleio_coord_outstanding.xlsx', index=False)


#We color the coordinates if they are common or only non pleio or pleio
n_important=100
...
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
# plt.scatter(0.005, 0.64, s=60, color='darkorange', alpha=0.6)  # s determina el tamaño del punto
# plt.text(0.05, 0.62, 'Non Pleio', fontsize=18)
...
# plt.scatter(0.005, 0.57, s=60, color='mediumseagreen', alpha=0.6)  # s determina el tamaño del punto
# plt.text(0.05, 0.55, 'Common', fontsize=18)
plt.grid(False)
...
df_over=pd.DataFrame({
    'OVER - pleio': ind_sorted_frac_genes_high_pleio_most_important, 
    'OVER - non-pleio': ind_sorted_frac_genes_less_pleio_most_important 
    })

df_over_coord_name=pd.DataFrame({
    'OVER - pleio': coord_high_pleio, 
    'OVER - non-pleio': coord_less_pleio
    })

df_over_frac=pd.DataFrame({
    'OVER - pleio': frac_genes_high_pleio_signif_per_coord_most_important, 
    'OVER - non-pleio': frac_genes_less_pleio_signif_per_coord_most_important
    })

df_over_n_cel=pd.DataFrame({
    'OVER - pleio': n_cell_high_pleio_most_important, 
    'OVER - non-pleio': n_cell_less_pleio_most_important
    })
...
# t_only_no_pleio=np.array(t_only_no_pleio, dtype=float)
t_only_pleio=np.array(t_only_pleio, dtype=float)

colors=['mediumslateblue', 'deepskyblue', 'hotpink']

pos=[0, 1, 2]
label=['Common', 'Non Pleio', 'Pleio']
plt.figure(figsize=(5, 5), dpi=600)
sns.violinplot([t_common, t_only_no_pleio, t_only_pleio],  palette=colors, inner='quartile', edgecolor='white', linewidth=1.5, cut=0)
plt.xticks(pos, label, fontsize=16, fontweight='bold')
plt.yticks(fontsize=18)
plt.grid(False)
plt.ylabel('Embryo times (min)', fontsize=20, fontweight='bold')
plt.xlabel('Genes', fontsize=16)
plt.savefig(path_save_data+'enriched_coord_times.png', dpi=600, bbox_inches='tight')
plt.show()
...
label=['Common', 'Non Pleio', 'Pleio']
plt.figure(figsize=(5, 5), dpi=600)
sns.violinplot([n_cel_common, n_cel_only_no_pleio, n_cel_only_pleio],  palette=colors, inner='quartile', edgecolor='white', linewidth=1.5, cut=0)
plt.xticks(pos, label, fontsize=16, fontweight='bold')
plt.yticks(fontsize=18)
plt.grid(False)
plt.ylabel('# cells per coordinate', fontsize=20, fontweight='bold')
plt.xlabel('Genes', fontsize=16)
plt.savefig(path_save_data+'enriched_coord_n_cell.png', dpi=600, bbox_inches='tight')
plt.show()
...
# unique_only_no_pleio, unique_only_no_pleio_n=np.unique(cel_only_no_pleio, return_counts=True)

#figure
plt.figure(figsize=(8, 5), dpi=600)
plt.bar(unique_only_pleio, unique_only_pleio_n, color='hotpink')
plt.grid(False)
plt.xticks(rotation=90, fontsize=18)
plt.title('Pleio genes', fontsize=22, fontweight='bold')
plt.ylabel('# coordinates', fontsize=22, fontweight='bold')
plt.yticks(fontsize=22)
plt.savefig(path_save_data+'enriched_coord_only_high_pleio_cell_types.png', dpi=600, bbox_inches='tight')
plt.show()

plt.figure(figsize=(8, 5), dpi=600)
plt.bar(unique_only_no_pleio, unique_only_no_pleio_n, color='deepskyblue')
plt.grid(False)
plt.xticks(rotation=90, fontsize=18)
plt.title('Non Pleio genes', fontsize=22, fontweight='bold')
plt.ylabel('# coordinates', fontsize=22, fontweight='bold')
plt.yticks(fontsize=22)
plt.savefig(path_save_data+'enriched_coord_only_less_pleio_cell_types.png', dpi=600, bbox_inches='tight')
plt.show()

plt.figure(figsize=(8, 5), dpi=600)
plt.bar(unique_common, unique_common_n, color='mediumslateblue')
plt.grid(False)
plt.xticks(rotation=90, fontsize=18)
plt.title('Common genes', fontsize=22, fontweight='bold')
plt.ylabel('# coordinates', fontsize=22, fontweight='bold')
plt.yticks(fontsize=22)
plt.savefig(path_save_data+'enriched_coord_common_pleio_and_not_cell_types.png', dpi=600, bbox_inches='tight')
plt.show()
...
for i in range(len(cell_types)):
    # if (len(np.where(unique_common==cell_types[i])[0])>0) or (len(np.where(unique_only_no_pleio==cell_types[i])[0])>0) or (len(np.where(unique_only_pleio==cell_types[i])[0])>0):
...
for i in range(len(m_types_array)):
    ...
    if m_types_array[i]>0:
        ...
        # if len(np.where(ind_non_pleio==i)[0])>0:
...
#figure
# cmap = ListedColormap(['white', 'gainsboro', 'mediumslateblue', 'deepskyblue', 'hotpink'])
...
# cbar.ax.set_yticklabels(['No sampled cells', 'Sampled cells', 'Common', 'Non Pleio', 'Pleio'], fontsize=15)
...
# sns.kdeplot(n_cell_types_pleio, color='mediumseagreen', fill=False, lw=1.5,  label='Pleio')
# sns.kdeplot(n_cell_types_non_pleio, color='darkorange', fill=False, lw=1.5,  label='Non-Pleio')

# sns.kdeplot(n_times_pleio, color='mediumseagreen', fill=False, lw=1.5,  label='Pleio')
# sns.kdeplot(n_times_non_pleio, color='darkorange', fill=False, lw=1.5,  label='Non-Pleio')
...
# ax.scatter(n_times_pleio, n_cell_types_pleio, n_coord_pleio, color='mediumseagreen', label='Pleio', alpha=0.5)
# ax.scatter(n_times_non_pleio, n_cell_types_non_pleio, n_coord_non_pleio, color='darkorange', label='Non-Pleio', alpha=0.5)
# ax.set_xlim([0, 1])  
# ax.set_ylim([1, 0])  
# ax.set_zlim([0, 1])  
# ax.set_zlabel('# Dev Coord')
# ax.set_xlabel('# Embryo times')
