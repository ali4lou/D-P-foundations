...
# #6.) KS test for each GO term with respect to the similiraties distributions
# #6.1.) We read the similarities distributions
# sim_dev=np.loadtxt(path_sim+'sim_dev_sum_cell_matrix_cosine.txt')
# sim_phen=np.loadtxt(path_sim+'sim_W_matrix_cosine.txt')

# #6.1.1) We compute the median of the sim_dev and sim_phen in each gene
# sim_dev_matrix=squareform(sim_dev)
# sim_phen_matrix=squareform(sim_phen)
...
# #6.2.) For each GO term we commpute the KS test 
# #We see if the distribution of the associted genes (with each go term) has a high or low KS test with respect the total distrib

# median_similarity=average_sim_phen_per_gen

# go_list_term=[GO_molecular, GO_bio_process, GO_cell_comp]
# go_list_term_descrip=[GO_molecular_descrip, GO_bio_process_descrip, GO_cell_comp_descrip]
# genes_label=['GO_molecular', 'GO_bio_process', 'GO_cell_comp']
# pathlist_use=[pathlist_molecular_unique, pathlist_bio_process_unique, pathlist_cell_comp_unique]

# common_genes=np.intersect1d(genes, genes_id_type)

# for i in range(len(go_list_term)):    
    
#     go_term_use=go_list_term[i]
#     go_term_use_descrip=go_list_term_descrip[i]
#     # go_term_use=GO_molecular
#     # go_term_use_descrip=GO_molecular_descrip
    
#     big_matrix=create_association_matrix_gene_go_term(genes, go_term_use, pathlist_use[i], go_term_use_descrip)
#     ks_stat=[]
#     p_val=[]
#     enriched_go_term=[]
#     enriched_go_term_descrip=[]
#     ks_stat_small=[]
#     p_val_small=[]
#     enriched_go_term_small=[]
#     enriched_go_term_descrip_small=[]
#     for j in range(len(go_term_use)):
#         index_genes_common=np.where(big_matrix[:, j]>0)[0]
#         if len(index_genes_common)>10:
#             genes_go_term_associted=common_genes[index_genes_common]
#             com, idx1, idx2 = np.intersect1d(genes, genes_go_term_associted, return_indices=True)
#             sim_go_term=median_similarity[idx1]
            
#             ks_statistic, p_value = kstest(median_similarity, sim_go_term,  alternative='greater') 
#             ks_statistic_small, p_value_small = kstest(median_similarity, sim_go_term,  alternative='less') 

        
#             ks_stat.append(ks_statistic)
#             p_val.append(p_value)
#             enriched_go_term.append(go_term_use[j])
#             enriched_go_term_descrip.append(go_term_use_descrip[j])
            
#             ks_stat_small.append(ks_statistic_small)
#             p_val_small.append(p_value_small)
#             enriched_go_term_small.append(go_term_use[j])
#             enriched_go_term_descrip_small.append(go_term_use_descrip[j])
        
        
#     #greater
#     p_val=np.array(p_val)
#     ks_stat=np.array(ks_stat)
#     enriched_go_term=np.array(enriched_go_term)
#     enriched_go_term_descrip=np.array(enriched_go_term_descrip)

#     index_sorted=np.argsort(p_val)

#     corrected_p_value=false_discovery_control(p_val, method='bh')

   
#     df_greater=pd.DataFrame()
#     df_greater['GO term']=enriched_go_term[index_sorted]
#     df_greater['GO description']=enriched_go_term_descrip[index_sorted]
#     df_greater['ks']=ks_stat[index_sorted]
#     df_greater['p-value']=p_val[index_sorted]
#     df_greater['corected p-value']=corrected_p_value[index_sorted]

#     df_greater.to_csv(path_save_data+'%s_KS_dev_sim_greater.csv' %genes_label[i], sep='\t')


#     #smaller
#     p_val_small=np.array(p_val_small)
#     ks_stat_small=np.array(ks_stat_small)
#     enriched_go_term_small=np.array(enriched_go_term_small)
#     enriched_go_term_descrip_small=np.array(enriched_go_term_descrip_small)

#     index_sorted=np.argsort(p_val_small)

#     corrected_p_value_small=false_discovery_control(p_val_small, method='bh')

   
#     df_small=pd.DataFrame()
#     df_small['GO term']=enriched_go_term_small[index_sorted]
#     df_small['GO description']=enriched_go_term_descrip_small[index_sorted]
#     df_small['ks']=ks_stat_small[index_sorted]
#     df_small['p-value']=p_val_small[index_sorted]
#     df_small['corected p-value']=corrected_p_value_small[index_sorted]

#     df_small.to_csv(path_save_data+'%s_KS_phen_sim_smaller.csv' %genes_label[i], sep='\t')
