...
path_save_data='C:\\Users\\logslab\\Desktop\\D-P_rule_paper\\NNMF_justification\\'
...
#7.) We read those matrices
path='C:\\Users\\logslab\\Desktop\\D-P_rule_paper\\NNMF_justification\\matrices_to_compare\\'
W_big=[]
H_big=[]
#voy a leer cada matriz de W y voy a calcular un valor pleiotrópico 
for i in range(n_run):
    W=np.loadtxt(path+'W_%d.txt' %i, dtype=float)
    W_big.append(W)
    H=np.loadtxt(path+'H_%d.txt' %i, dtype=float)
    H_big.append(H)
